...
ChippyCpu::ChippyCpu()
{
	...
	{
		memory_[SPRITE_SET_START + i] = sprites_[i];
	}
}
...
void ChippyCpu::LoadProgram(char *buffer, size_t size)
{
	for (size_t i = 0; i < size; i++)
	{
		memory_[i + PROGRAM_START] = buffer[i];
	}
}
...
void ChippyCpu::Opcode()
{
	...
	/* Common variables */
	unsigned int x = (instruction & 0x0F00) >> 8; /* Index used for V[x] register */
	unsigned int y = (instruction & 0x00F0) >> 4; /* Index used for V[y] register */
	...
	case 0xD000: /* Display n-byte sprite starting at memory location I at (Vx, Vy), set VF = collision. */
	{
		...
		std::uint8_t x_pos = static_cast<std::uint8_t>(v_register_[x] % INTERNAL_WIDTH);
		std::uint8_t y_pos = static_cast<std::uint8_t>(v_register_[y] % INTERNAL_HEIGHT);
		...
		for (size_t i = 0; i < height; i++)
		{
			...
			for (size_t j = 0; j < 8; j++)
			{
				...
				std::uint32_t *display_pixel = &display_.data()[(y_pos + i) * INTERNAL_WIDTH + (x_pos + j)];
	...
	case 0xF000:
	{
		...
		case 0x000A: /* Wait for a key press, store the value of the key in Vx. */
		{
			...
			for (size_t i = 0; i < keyboard_.size(); i++)
			{
				...
				{
					key_pressed = true;
					v_register_[x] = static_cast<std::uint8_t>(i);
				}
			}
		...
		case 0x0029: /* Set I = location of sprite for digit Vx. */
			i_register_ = (v_register_[x] * 5) + SPRITE_SET_START;
			break;
